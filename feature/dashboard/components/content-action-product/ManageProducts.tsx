              ...
              <tbody className="divide-y">
                {filteredProducts.map((p) => (
                  <tr key={p.id} className="hover:bg-gray-50">
                    <td className="p-2 font-semibold">{p.name || p.productName}</td>
                    <td className="p-2">{p.category?.name || p.categoryName || "-"}</td>
                    <td className="p-2">{formatRupiah(p.sellPrice || p.priceSell || 0)}</td>
                    <td className="p-2 text-green-600">{formatRupiah(p.cleanProfit || 0)}</td>
                    <td className="p-2">
                    ...
                    <td className="p-2 text-right">
                      <div className="flex justify-end gap-2 items-center">
                        <AddProduct mode="edit" initialData={p} className="h-8 px-2 text-xs" />
                        <Button
                          variant="destructive"
                          size="sm"
